...
class LambdaScheduler:
    ...
    def __init__(self, policy:str="GD", mem_capacity:int=32000, num_funcs:int=10, run:str="a", log_dir=""):
        # log_dir = "/Users/xieshuaiyu/experiment/faascache-sim/data2/trace_pckl/middle/logs/"
        ...
        self.cold_start = 0 # number of cold starts
    ...
    def PreWarmContainers(self):
        ...
        if self.eviction_policy != "HIST":
            return
        to_warm = [kind for kind, prewarm in self.histPrewarm.items() if prewarm != 0 and prewarm + self.last_seen[kind] >= self.wall_time]  # 这里是不是写错了，应该是prewarm + self.last_seen[kind] < self.wall_time
        for kind in to_warm:
    ...
    def RemoveFromPool(self, c: Container, reason: str):
        ...
        self.mem_cost += c.metadata.mem_size * (self.wall_time - c.keep_alive_start_t) # 累计开销
    ...
    def Eviction(self, d: LambdaData):
        ...
        for v in eviction_list:
            ...
            k = v.metadata.kind
            self.evdict[k] += 1
            self.mem_cost += v.metadata.mem_size * (self.wall_time - v.keep_alive_start_t) # 累计开销
    ...
    def cache_miss(self, d:LambdaData, keep_alive_TTL = 0):

        c = Container(d,keep_alive_TTL) if self.eviction_policy == "LONG-TTL" else Container(d)
    ...
    def runActivation(self, d: LambdaData, t = 0, action = 10):
        ...
        if c is None:
            ...
            c = self.cache_miss(d, action)
            if c is None:
                # insufficient memory
                self.capacity_misses[d.kind] += 1
                return False, is_hit, increase_time
            c.run()
            #Need to update priority here?
            processing_time = d.run_time
            increase_time = d.run_time - d.warm_time    # increased time for cold start compared with warm start
        ...
        heapq.heapify(self.ContainerPool)
        return True, is_hit, increase_time
    ##############################################################
...
if __name__ == "__main__":
    ...
    pth = "/Users/xieshuaiyu/experiment/faascache-sim/data2/trace_pckl/bottom_qt/10-b.pckl"
